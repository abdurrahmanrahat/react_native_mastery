...
import { TFavoriteRecipe } from "@/types/recipe";
...
const RecipeDetailScreen = () => {
  ...
  useEffect(() => {
    const checkIfSaved = async () => {
      try {
        ...
        const isRecipeSaved = favorites.some(
          (fav: TFavoriteRecipe) => fav.recipeId === parseInt(id as string),
        );
    ...
    const loadRecipeDetail = async () => {
      ...
      try {
        const mealData = await MealApis.getMealById(id as string);
        if (mealData) {
          const transformedRecipe = MealApis.transformMealData(mealData);
